...
def open_files(*args):
    """
    Funcion para abrir los archivos que seran procesados,
    en caso de que no exista el archivo lo notificara pero no finalizara.
    *args -> Archivos a los que se les dara formato
    """
    for item in args:
        try:
            file = open(item,'rt')
            ...
            print('Ocurrio un error al abrir el archivo',item)


def formato(item,texto):
    """
    Funcion para dar formato a los archivos de LISA
    Formato:
        <num> | <text>
        <num2> | <text2>
    """
    ...
    new_cadena=''
    for e in texto:
        if e in limitadores and new_cadena!='': # * #
            new_cadena=new_cadena+'\n'
            new_file.write(new_cadena)
            new_cadena=''
        elif e!='\n' and e not in limitadores:
            if new_cadena[-1:] in numeros and e not in numeros:
                new_cadena=new_cadena+'|'+e 
            elif len(new_cadena)==0 and e not in numeros:
                continue
            else:
                new_cadena=new_cadena+e
